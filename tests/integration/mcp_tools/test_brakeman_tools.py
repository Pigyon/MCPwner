"""Integration tests for Brakeman MCP tools."""
...
@pytest.mark.asyncio
async def test_sast_brakeman_scan_invalid_workspace(docker_compose_up):
    """Test sast_brakeman_scan with invalid workspace_id."""
...
@pytest.mark.asyncio
async def test_sast_brakeman_get_report_invalid_workspace(docker_compose_up):
    """Test sast_brakeman_get_report with invalid workspace_id."""
    async with create_mcp_client() as session:
        result = await session.call_tool(
            "sast_brakeman_get_report",
            arguments={"workspace_id": "nonexistent-workspace"},
        )
