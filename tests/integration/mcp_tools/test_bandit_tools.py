"""Integration tests for Bandit MCP tools."""
...
@pytest.mark.asyncio
async def test_sast_bandit_scan_invalid_workspace(docker_compose_up):
    """Test sast_bandit_scan with invalid workspace_id."""
    async with create_mcp_client() as session:
        ...
        print("\n✅ sast_bandit_scan error handling for invalid workspace works")


@pytest.mark.asyncio
async def test_sast_bandit_get_report_invalid_workspace(docker_compose_up):
    """Test sast_bandit_get_report with invalid workspace_id."""
    async with create_mcp_client() as session:
        result = await session.call_tool(
        ...
        print("\n✅ sast_bandit_get_report error handling for invalid workspace works")
