"""Integration tests for PMD MCP tools."""
...
@pytest.mark.asyncio
async def test_sast_pmd_scan_invalid_workspace(docker_compose_up):
    """Test sast_pmd_scan with invalid workspace_id."""
    async with create_mcp_client() as session:
        ...
        print("\n✅ sast_pmd_scan error handling for invalid workspace works")


@pytest.mark.asyncio
async def test_sast_pmd_get_report_invalid_workspace(docker_compose_up):
    """Test sast_pmd_get_report with invalid workspace_id."""
    async with create_mcp_client() as session:
        result = await session.call_tool(
            "sast_pmd_get_report",
            arguments={"workspace_id": "nonexistent-workspace"},
        ...
        print("\n✅ sast_pmd_get_report error handling for invalid workspace works")
