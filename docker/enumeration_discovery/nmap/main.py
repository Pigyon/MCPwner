    ...
    try:
        ...
        if "workspaces" in parts:
            ...
            if idx + 1 < len(parts):
                ...
                output_dir = workspace_root / "reports" / "enumeration_discovery" / TOOL_NAME
            else:
                output_dir = (
                    Path(request.workspace_path) / "reports" / "enumeration_discovery" / TOOL_NAME
                )
        else:
            output_dir = (
                Path(request.workspace_path).parent / "reports" / "enumeration_discovery" / TOOL_NAME
            )
