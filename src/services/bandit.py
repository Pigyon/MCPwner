...
class BanditService:
    ...
    def __init__(self, repository: WorkspaceRepository, bandit_client: BanditClient):
        ...
        self.bandit_client = bandit_client
    ...
    def scan(
        ...
        """
        Execute Bandit scan on workspace.
    ...
    def get_latest_report(self, workspace_id: str) -> Dict[str, Any]:
        """
        Get the most recent Bandit report for a workspace.
        ...
        report_dir = Path(f"/workspaces/{workspace_id}/reports/sast/bandit")
        if not report_dir.exists():
            return {
                "status": "error",
                "error": f"No Bandit reports found for workspace: {workspace_id}",
    ...
    def is_language_supported(self, language: str) -> bool:
        """
        Check if language is supported by Bandit.
