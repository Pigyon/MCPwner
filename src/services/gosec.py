...
class GosecService:
    ...
    def scan(
        ...
        """
        Execute Gosec scan on workspace.

        Args:
            workspace_id: UUID of the workspace to scan
            scan_path: Optional relative path within workspace to scan
            config: Optional Gosec configuration (severity, confidence, exclude patterns)
        ...
        try:
            ...
            return {
                "status": "error",
                "error": f"Gosec scan failed: {e}",
                "error_code": "SCAN_FAILED",
            }

    def get_latest_report(self, workspace_id: str) -> Dict[str, Any]:
        """
        Get the most recent Gosec report for a workspace.
        ...
        report_dir = Path(f"/workspaces/{workspace_id}/reports/sast/gosec")
        if not report_dir.exists():
            return {
                "status": "error",
                "error": f"No Gosec reports found for workspace: {workspace_id}",
    ...
    def is_language_supported(self, language: str) -> bool:
        """
        Check if language is supported by Gosec.
