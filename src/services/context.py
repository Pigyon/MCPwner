...
class ContextService:
    ...
    def __init__(self, repository: WorkspaceRepository, codeql_bin: str = "codeql"):
        self.repository = repository
        self.extractor = ContextExtractorService(codeql_bin)

    def extract_context(
        self, workspace_id: str, database_id: str, base_path: str = "/workspaces"
    ) -> Dict[str, Any]:
        ...
        context_db_path = str(Path(base_path) / workspace_id / "context" / f"{database.language}.db")

        # Extract functions
        functions_result = self.extractor.extract_functions(
            database.path, context_db_path, database.language
        )
        ...
        return {
            ...
            "duration_seconds": (
                functions_result["duration_seconds"] + callgraph_result.get("duration_seconds", 0)
            ),
        }
    ...
    def get_function_context(
        ...
        context_db_path = str(Path(base_path) / workspace_id / "context" / f"{language}.db")
