...
class BrakemanService:
    ...
    def scan(
        ...
        """
        Execute Brakeman scan on workspace.
        ...
        try:
            result = self.brakeman_client.scan(
            ...
            return {
                "status": "error",
                "error": f"Brakeman scan failed: {e}",
                "error_code": "SCAN_FAILED",
            }

    def get_latest_report(self, workspace_id: str) -> Dict[str, Any]:
        """
        Get the most recent Brakeman report for a workspace.
        ...
        report_dir = Path(f"/workspaces/{workspace_id}/reports/sast/brakeman")
        if not report_dir.exists():
            return {
                "status": "error",
                "error": f"No Brakeman reports found for workspace: {workspace_id}",
    ...
    def is_language_supported(self, language: str) -> bool:
        """
        Check if language is supported by Brakeman.
