...
def sast_pmd_scan(
    ...
    """
    Execute PMD SAST scan on a workspace.
    ...
    """
    print(
        f"[MCP SERVER] sast_pmd_scan called: workspace_id={workspace_id}, scan_path={scan_path}",
        file=sys.stderr,
    )
    ...
    try:
        ...
        if result.get("status") == "success":
            print(
                f"[MCP SERVER] PMD scan completed: {result.get('finding_count', 0)} findings",
                file=sys.stderr,
            )
        else:
            print(
                f"[MCP SERVER] PMD scan failed: {result.get('error', 'Unknown error')}",
                file=sys.stderr,
            )

        return result
    except Exception as e:
        error_msg = f"PMD scan failed: {e}"
