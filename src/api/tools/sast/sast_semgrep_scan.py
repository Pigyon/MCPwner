...
def sast_semgrep_scan(
    ...
    """
    print(
        f"[MCP SERVER] sast_semgrep_scan called: workspace_id={workspace_id}, scan_path={scan_path}",
        file=sys.stderr,
    )
    ...
    try:
        ...
        if result.get("status") == "success":
            print(
                f"[MCP SERVER] Semgrep scan completed: {result.get('finding_count', 0)} findings",
                file=sys.stderr,
            )
        else:
            print(
                f"[MCP SERVER] Semgrep scan failed: {result.get('error', 'Unknown error')}",
                file=sys.stderr,
            )
