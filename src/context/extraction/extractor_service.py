...
class ContextExtractorService:
    ...
    def extract_functions(
        self, database_path: str, context_db_path: str, language: str
    ) -> Dict[str, Any]:
        """
        ...
        # Parse results
        parsed_rows = parse_csv_results(result["output_file"], adapter.parse_function_result)
    ...
    def extract_call_graph(
        ...
        # Parse results
        parsed_rows = parse_csv_results(result["output_file"], adapter.parse_call_graph_result)
        ...
        for row in parsed_rows:
            try:
                ...
                caller = repo.code_elements.get_by_name(row["caller_name"], row["caller_file"])
                callee = repo.code_elements.get_by_name(row["callee_name"], row["callee_file"])
