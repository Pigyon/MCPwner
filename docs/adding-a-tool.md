...
This walkthrough uses a hypothetical SAST scanner called `newscanner` on port `8097`.
...
EXPOSE 8097
CMD ["python", "-m", "uvicorn", "main:app", "--host", "0.0.0.0", "--port", "8097"]
```
...
```yaml
  newscanner:
    ...
    healthcheck:
      test: ["CMD", "python3", "-c", "import urllib.request; urllib.request.urlopen(\"http://localhost:8097/health\")"]
      interval: 60s
      timeout: 10s
      retries: 3
      start_period: 10s
...
> profile and it starts even for unrelated workflows. Static/dynamic utilities are gated exactly
> this way.
...
```python
_spec("newscanner", _SAST, ("newscanner",), "http://newscanner:8097"),
```
...
# SAST Services
newscanner:
  service_url: "http://newscanner:8097"
```
